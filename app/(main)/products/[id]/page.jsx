...
const page =  () => {
  ...
  const fetchDetails = async () => {
    try {
      const {data} = await axios.get(`https://shopysam-back.onrender.com/api/product/details/${params?.id}`)
      if (data?.success) {
        setData(data?.data)
      }
    } catch (error) {
            ...
            <div>
              ...
              <div className='flex items-center gap-2 lg:gap-4'>
                <div className='border border-green-600 px-4 py-2 rounded bg-green-50 w-fit'>
                    <p className='font-semibold text-lg lg:text-xl'>{pricewithDiscount(data?.price,data?.discount)}</p>
                </div>
                {
                  data?.discount && (
